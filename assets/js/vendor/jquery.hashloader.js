    ...
    if (newHash) {
      ...
      if (hashDestination.data("destination")) {
        $.get(newHash, function (data) {
           ...
           */
          if ($(".portfolio-item-details").length != 0 &&
            hashDestination.data("destination") ==
            'portfolio-items') {
          ...
          if (hashDestination.data("insert") == "before") {
            $(data).insertBefore("." + hashDestination.data(
              "destination"));
          } else if (hashDestination.data("insert") == "after") {
            $(data).insertAfter("." + hashDestination.data(
              "destination"));
          } else if (hashDestination.data("insert") ==
            "prependTo") {
            $(data).prependTo("." + hashDestination.data(
              "destination"));
          } else if (hashDestination.data("insert") ==
            "appendTo") {
            $(data).appendTo("." + hashDestination.data(
              "destination"));
          } else if (hashDestination.data("insert") == "html") {
            $("." + hashDestination.data("destination")).html(
              data);
          } else {
            $(data).append("." + hashDestination.data(
              "destination"));
          }
        }).complete(function () {
          /*!
           ...
           */
          if (hashDestination.data("destination") ==
            'plugin-filter-elements') {
            setTimeout(function () {
              $('.plugin-filter-elements').mixitup('remix',
                'all');
            }, 1000);
          }
